...
class Ticker(db.Model):
    ...
    ticker = db.Column(db.String(10), unique=True, nullable=False, index=True)  # Ajout index
    isin = db.Column(db.String(12), unique=True, nullable=True)
    name = db.Column(db.String(255), nullable=False)
    country = db.Column(db.String(50), nullable=False)
    sector = db.Column(db.String(100), nullable=False, index=True)  # Ajout index
    market_cap = db.Column(db.BigInteger, nullable=True)
    currency = db.Column(db.String(3), nullable=False)
    exchange = db.Column(db.String(10), nullable=False)
    index_membership = db.Column(db.String(50), nullable=True, index=True)  # Ajout index
